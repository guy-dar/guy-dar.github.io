...
A minimal implementation is provided in: [https://github.com/guy-dar/speaking-probes](https://github.com/guy-dar/speaking-probes). 
...
We rely on a useful view of the transformer through its residual connections originally introduced in [nostalgebraist (2020)](https://www.lesswrong.com/posts/AcKRB8wDpdaN6v6ru/interpreting-gpt-the-logit-lens). Specifically, each layer takes a hidden state as input and adds information to the hidden state through its residual connection. Under this view, the hidden state is a residual stream passed along the layers, from which information is read, and to which information is written at each layer. [Elhage et al. (2021)](https://transformer-circuits.pub/2021/framework/index.html) and [Geva et al. (2022b)](https://arxiv.org/abs/2203.14680) observed that the residual stream is often barely updated in the last layers, and thus the final prediction is determined in early layers and the hidden state is mostly passed through the later layers. An exciting consequence of the residual stream view is that we can project hidden states in every layer into embedding space by multiplying the hidden state with the embedding matrix E, treating the hidden state as if it were the output of the last layer. [Geva et al. (2022a)](https://arxiv.org/abs/2204.12130) used this approach to interpret the prediction of transformer-based language models, and we will follow a similar approach.
...
We will focus on parameters in this article, as hidden states seem to be more complicated to analyze - which stands to reason since they are mixtures of parameters. We will see that parameter vectors can be used alongside token embeddings in the input prompt and produce meaningful responses. From the examples I analyzed, it seems that when interpreting a neuron with speaking probes, it "decides" on one of the concepts it encodes and eventually collapses on it.
...
In general, there is not much research on continuous vectors as first-class citizens in transformers. While ideas like prompt tuning ([Lester et al., 2021](https://arxiv.org/abs/2104.08691)), and exciting ideas like [Hao et al. (2022)](https://arxiv.org/abs/2206.06336) pass continuous inputs to the model, they require training to work and they are not used zero-shot. A central motif in this work is the investigation of whether some continuous vectors can be used like natural tokens without further training - under the assumption that they use the same "language" as the model.
...
* **Neuron Polysemy**: Especially in the face of word collapse, it seems that neurons that carry multiple unrelated interpretations will have to be sampled multiple times to account for all their different meanings. We would like to be able to extract the different meanings more faithfully and "in one sitting".

* **Better Prompts**: This is not the main part of our work, but many papers show the benefits of using carefully engineered prompts (e.g., [Liu et al., 2021](https://arxiv.org/abs/2101.06804)).
